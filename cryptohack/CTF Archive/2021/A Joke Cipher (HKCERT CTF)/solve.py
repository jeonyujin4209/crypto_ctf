"""
A Joke Cipher (HKCERT CTF 2021)

The Nagaty cryptosystem "shared key" is (y_A * y_B)^2 mod p.

Alice's exchange:  S_AB_A = (y_A * y_B * y_B) * S_A mod p
Since S_A mod p = y_A: y_A * S_A ≡ y_A^2 (mod p)
So S_AB_A ≡ y_A^2 * y_B^2 ≡ (y_A * y_B)^2 (mod p)

Encryption is plaintext * shared_key (no modular reduction).
So flag = c // shared_key.
"""
from Crypto.Util.number import long_to_bytes

p  = 
y_A = 
y_B = 
c  = 

SK = pow(y_A * y_B % p, 2, p)
flag_int = c // SK
flag = long_to_bytes(flag_int)
print(flag)

"""
Unimplemented (TETCTF 2021)

Complex RSA: n = p^2 * q^2, e = 65537, plaintext is a Gaussian integer.
The output gives private_key = (p, q) directly!

decrypt(c) needs to compute c^d mod n in Z[i]/(n) where d = e^{-1} mod lambda(n).

For Z[i]/(p^2) with p, q from private_key:
  p % 4 == 1: Z[i]/(p) ≅ GF(p) × GF(p), lambda = p*(p-1) (exponent p(p-1))
  p % 4 == 3: Z[i]/(p) ≅ GF(p^2), lambda = p*(p^2-1) for Z[i]/(p^2)

Full lambda = lcm of all component lambdas.
"""
...
p = 128329415694646850105527417663220454989310213490980740842294900866469518550360977403743209328130516433033852724185671092403884337579882897537139175073013
q = 119773890850600188123646882522766760423725010264224559311769920026142724028924588464361802945459257237815241227422748585976629359167921441645714382651911
n = (p**2) * (q**2)
e = 65537
...
c = Complex(
    int.from_bytes(ciphertext[:half], 'big'),
    int.from_bytes(ciphertext[half:], 'big'),
)

# Compute lambda(n) = lcm of component exponents
# p ≡ 1 mod 4 → lambda for Z[i]/(p^2) is p*(p-1)
# q ≡ 3 mod 4 → lambda for Z[i]/(q^2) is q*(q^2-1)
print(f"p mod 4 = {p % 4}")
print(f"q mod 4 = {q % 4}")

# p % 4 == 1: Z[i]/(p^2) ≅ Z/(p^2) × Z/(p^2), exponent = p*(p-1)
# q % 4 == 3: Z[i]/(q^2) local ring, exponent = q*(q^2-1)
lambda_p2 = p * (p - 1)
lambda_q2 = q * (q * q - 1)
lam = lcm(lambda_p2, lambda_q2)

d = pow(e, -1, lam)
m = complex_pow(c, d, n)

# Unpad: pad_data + b'\x00' + data
raw_re = m.re.to_bytes((m.re.bit_length() + 7) // 8, 'big')
raw_im = m.im.to_bytes((m.im.bit_length() + 7) // 8, 'big')

full_plaintext = raw_re + raw_im
print("plaintext (raw):", full_plaintext)

# unpad
if b'\x00' in full_plaintext:
    flag = full_plaintext.split(b'\x00', 1)[1]
    print("flag:", flag)

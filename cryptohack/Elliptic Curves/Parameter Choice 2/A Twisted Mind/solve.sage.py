...
_sage_const_2 = Integer(2); _sage_const_192 = Integer(192); _sage_const_237 = Integer(237); _sage_const_3 = Integer(3); _sage_const_1379137549983732744405137513333094987949371790433997718123 = Integer(1379137549983732744405137513333094987949371790433997718123); _sage_const_6277101735386680763835789423072729104060819681027498877478 = Integer(6277101735386680763835789423072729104060819681027498877478); _sage_const_1 = Integer(1); _sage_const_11 = Integer(11); _sage_const_269 = Integer(269); _sage_const_2985973 = Integer(2985973); _sage_const_1631363213 = Integer(1631363213); _sage_const_123462957881 = Integer(123462957881); _sage_const_1607 = Integer(1607); _sage_const_693493 = Integer(693493); _sage_const_1483406443 = Integer(1483406443); _sage_const_40202856427 = Integer(40202856427); _sage_const_0 = Integer(0); _sage_const_4 = Integer(4); _sage_const_5 = Integer(5); _sage_const_17 = Integer(17); _sage_const_12345 = Integer(12345); _sage_const_13416 = Integer(13416); _sage_const_50 = Integer(50); _sage_const_4096 = Integer(4096)
...
N  = _sage_const_6277101735386680763835789423072729104060819681027498877478 
Np = _sage_const_2 *(p+_sage_const_1 ) - N
...
N_smooth_factors = [_sage_const_2 , _sage_const_11 , _sage_const_269 , _sage_const_2985973 , _sage_const_1631363213 , _sage_const_123462957881 ]
# Drop 3^3 (prime-power ambiguity when target order < 27) — use only 3.
Np_smooth_factors = [_sage_const_2 , _sage_const_3 , _sage_const_1607 , _sage_const_693493 , _sage_const_1483406443 , _sage_const_40202856427 ]

N_smooth = _sage_const_1 
for f in N_smooth_factors:
    N_smooth *= f

Np_smooth = _sage_const_1 
...
Et = EllipticCurve(F, [a * u**_sage_const_2 , b * u**_sage_const_3 ])
assert Et.order() == Np
...
print(f"[+] P_E = ({P_E[_sage_const_0 ]}, {P_E[_sage_const_1 ]})  x0 = {P_E[_sage_const_0 ]}")
...
print(f"[+] P_Et = ({P_Et[_sage_const_0 ]}, {P_Et[_sage_const_1 ]})")
# Map to E-twist x-coord: x_on_E_twist = x_t / u
P_Et_x_on_E = F(P_Et[_sage_const_0 ]) / u
...
def _dbl_XZ(P1):
    ...
    XX = (X1 * X1) % pL
    ZZ = (Z1 * Z1) % pL
    A = (_sage_const_2  * ((X1 + Z1) * (X1 + Z1) - XX - ZZ)) % pL
    aZZ = (aL * ZZ) % pL
    X3 = ((XX - aZZ) * (XX - aZZ) - _sage_const_2  * bL * A * ZZ) % pL
    Z3 = (A * (XX + aZZ) + _sage_const_4  * bL * ZZ * ZZ) % pL
    return (X3, Z3)
...
def _diffadd_XZ(P1, P2, x0):
    ...
    X3 = (_sage_const_2  * T + _sage_const_4  * bL * Z1Z2 * Z1Z2 - x0 * Z3) % pL
    return (X3, Z3)
...
def scalarmult_E_x(k, x0):
    ...
    if bit:
        R0 = R1
    if R0[_sage_const_1 ] == _sage_const_0 :
        return "Infinity"
    return (int(R0[_sage_const_0 ]) * pow(int(R0[_sage_const_1 ]), -_sage_const_1 , pL)) % pL
...
X1 = int(P_E[_sage_const_0 ])
Z1 = _sage_const_1 
XX = (X1 * X1) % p_py
ZZ = _sage_const_1 
A_coef = (_sage_const_2  * ((X1 + Z1)**_sage_const_2  - XX - ZZ)) % p_py
aZZ = (a_py * ZZ) % p_py
X3 = ((XX - aZZ)**_sage_const_2  - _sage_const_2  * b_py * A_coef * ZZ) % p_py
Z3 = (A_coef * (XX + aZZ) + _sage_const_4  * b_py * ZZ**_sage_const_2 ) % p_py
two_P_x = (X3 * pow(Z3, -_sage_const_1 , p_py)) % p_py
print(f"  inline   2P.x = {two_P_x}")
print(f"  Sage     2P.x = {int((_sage_const_2 *P_E)[_sage_const_0 ])}")

# Now test scalarmult_E_x with plain ints
got = scalarmult_E_x(_sage_const_2 , int(P_E[_sage_const_0 ]))
print(f"  ladder 2P.x = {got}")

for k_test in [_sage_const_1 , _sage_const_2 , _sage_const_5 , _sage_const_17 , _sage_const_12345 ]:
    expected = int((int(k_test) * P_E)[_sage_const_0 ])
    got = scalarmult_E_x(k_test, int(P_E[_sage_const_0 ]))
...
x0_twist = int(F(P_Et[_sage_const_0 ]) / u)
for k_test in [_sage_const_1 , _sage_const_2 , _sage_const_5 , _sage_const_17 , _sage_const_12345 ]:
    expected = int(F((k_test * P_Et)[_sage_const_0 ]) / u)
...
PORT = int(_sage_const_13416 )  # force plain Python int — Sage preparser would otherwise give Integer
...
def solve():
    ...
    try:
        while b"\n" not in greet or len(greet) < _sage_const_50 :
            greet += sock.recv(_sage_const_4096 )
    except Exception:
        pass
    ...
    for f in N_smooth_factors:
        ...
        if dQ_x == "Infinity":
            residues_E.append((_sage_const_0 , f))
    ...
    for f in Np_smooth_factors:
        ...
        if dQ2_x_on_E == "Infinity":
            residues_Et.append((_sage_const_0 , f))
            print(f"  factor {f}: k = 0 (Q·d = O)")
            continue
    ...
    def crt_list(pairs):
        ...
        x, m = _sage_const_0 , _sage_const_1 
        for (r, f) in pairs:
            # Merge x mod m with r mod f
            g = gcd(m, f)
            if (r - x) % g != _sage_const_0 :
                return None
            lcm_mf = (m * f) // g
            diff = (r - x) // g
            m_over_g = m // g
            f_over_g = f // g
            inv = pow(m_over_g, -_sage_const_1 , f_over_g)
    ...
    order_half = N // _sage_const_2 
    all_res = residues_E + residues_Et
    if None in all_res:
        print("[!] some subgroup DL failed")
        return
    for signs in product([_sage_const_1 , -_sage_const_1 ], repeat=len(all_res)):
        ...
        for cand in (x, M - x):
            if _sage_const_0  < cand <= order_half:

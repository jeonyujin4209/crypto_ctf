"""Test per-byte accuracy with uniform allocation (no elimination) vs sequential halving."""

from Crypto.Util.Padding import unpad
from Crypto.Cipher import AES
from os import urandom
from random import SystemRandom
import os
import random

rng_local = SystemRandom()
HEX_LIST = [ord(c) for c in '0123456789abcdef']


class LocalChallenge:
    def __init__(self):
        self.message = urandom(16).hex()
        self.key = urandom(16)
        self.query_count = 0

    def get_ct(self):
        iv = urandom(16)
        cipher = AES.new(self.key, AES.MODE_CBC, iv=iv)
        ct = cipher.encrypt(self.message.encode("ascii"))
        return (iv + ct).hex()

    def check_padding(self, ct_hex):
        ct = bytes.fromhex(ct_hex)
        iv, ct = ct[:16], ct[16:]
        cipher = AES.new(self.key, AES.MODE_CBC, iv=iv)
        pt = cipher.decrypt(ct)
        try:
            unpad(pt, 16)
        except ValueError:
            good = False
        else:
            good = True
        self.query_count += 1
        return good ^ (rng_local.random() > 0.4)


def make_ct(target, inter, pos, pad, cand):
    m = bytearray(os.urandom(16))
    for k in range(pos + 1, 16):
        m[k] = inter[k] ^ pad
    m[pos] = cand ^ pad
    return (bytes(m) + bytes(target)).hex()


def find_byte_uniform(challenge, target, inter, pos, prev_byte, budget):
    """Uniform allocation: distribute budget evenly across all 16 candidates."""
    pad = 16 - pos
    cands = [prev_byte ^ h for h in HEX_LIST]

    scores = [0] * 16
    used = 0
    q_each = budget // 16

    for idx in range(16):
        for _ in range(q_each):
            r = challenge.check_padding(make_ct(target, inter, pos, pad, cands[idx]))
            scores[idx] += (-1 if r else 1)
            used += 1

    best = max(range(16), key=lambda i: scores[i])
    return cands[best], used


def find_byte_seqhalf(challenge, target, inter, pos, prev_byte, budget):
    """Sequential halving."""
    pad = 16 - pos
    cands = [prev_byte ^ h for h in HEX_LIST]

    scores = [0] * 16
    used = 0
    active = list(range(16))

    # Better round structure: more queries in early rounds
    rounds = [(6, 8), (12, 4), (24, 2)]

    for q_each, keep in rounds:
        for idx in active:
            for _ in range(q_each):
                if used >= budget:
                    break
                r = challenge.check_padding(make_ct(target, inter, pos, pad, cands[idx]))
                scores[idx] += (-1 if r else 1)
                used += 1
        active.sort(key=lambda i: scores[i], reverse=True)
        active = active[:keep]

    while used < budget:
        for idx in active:
            if used >= budget:
                break
            r = challenge.check_padding(make_ct(target, inter, pos, pad, cands[idx]))
            scores[idx] += (-1 if r else 1)
            used += 1

    best = max(active, key=lambda i: scores[i])
    return cands[best], used


def find_byte_seqhalf2(challenge, target, inter, pos, prev_byte, budget):
    """Improved sequential halving with more queries per round."""
    pad = 16 - pos
    cands = [prev_byte ^ h for h in HEX_LIST]

    scores = [0] * 16
    used = 0
    active = list(range(16))

    # Round 1: 8q × 16 = 128 → keep 4
    # Round 2: rest (~remaining) between 4 → keep 1
    # Actually: 16 → 4 directly, then final
    q1 = min(8, budget // 16)
    for idx in active:
        for _ in range(q1):
            if used >= budget:
                break
            r = challenge.check_padding(make_ct(target, inter, pos, pad, cands[idx]))
            scores[idx] += (-1 if r else 1)
            used += 1

    active.sort(key=lambda i: scores[i], reverse=True)
    active = active[:4]

    # Remaining budget between top 4
    while used < budget:
        for idx in active:
            if used >= budget:
                break
            r = challenge.check_padding(make_ct(target, inter, pos, pad, cands[idx]))
            scores[idx] += (-1 if r else 1)
            used += 1

    best = max(active, key=lambda i: scores[i])
    return cands[best], used


# Test
for budget in [75, 100, 125, 150, 200, 250, 370]:
    results = {}
    n_trials = 500

    for method_name, method in [("uniform", find_byte_uniform),
                                  ("seqhalf2", find_byte_seqhalf2)]:
        correct = 0
        for _ in range(n_trials):
            ch = LocalChallenge()
            ct_hex = ch.get_ct()
            ct_b = bytes.fromhex(ct_hex)
            c1 = list(ct_b[16:32])
            c2 = list(ct_b[32:48])

            cipher_ecb = AES.new(ch.key, AES.MODE_ECB)
            true_inter = list(cipher_ecb.decrypt(bytes(c2)))

            pos = random.randint(0, 15)
            best, _ = method(ch, c2, true_inter, pos, c1[pos], budget)
            if best == true_inter[pos]:
                correct += 1

        results[method_name] = correct

    print(f"Budget={budget:3d}: " + "  ".join(f"{k}={100*v/n_trials:.1f}%" for k, v in results.items()))
